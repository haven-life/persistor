...
export namespace Mongo {
    ...
    export async function persistSave(persistor, obj, promises, masterId, idMap, txn, logger?) {
        ...
        for (var prop in props) {
            ...
            var isCrossDocRef = Schema.isCrossDocRef(template, prop, defineProperty);
            var value = obj[prop];
        ...
        if (savePOJO) {
            promises.push(await savePojoToMongo(persistor, obj, pojo, isDocumentUpdate ? new ObjectID(obj._id) : null, txn, logger));
        }

        // @TODO: we may not resolvePromises, is that alright?
        if (resolvePromises) {
            let resolvedPojo = await UtilityFunctions.resolveRecursivePromises(promises, pojo);
    ...
    export async function getTemplateFromPOJO(persistor, pojo, template, promises, defineProperty, idMap, cascade, establishedObj, specificProperties, isTransient, logger?) {
        ...
        // if(!pojo._template) {
        //     console.log("WE DO NOT HAVE A FUCKING TEMPLATE....WHY", pojo);
        // }

        var obj = establishedObj || idMap[pojo._id.toString()] ||
            persistor._createEmptyObject(template, 'persist' + template.__name__ + '-' + pojo._template.replace(/.*:/, '') +
        ...
        for (var prop in props) {
            ...
            var isCrossDocRef = Schema.isCrossDocRef(obj.__template__, prop, defineProperty) || defineProperty.autoFetch;
    ...
    export async function getPOJOByQuery(persistor, template, query, options, logger?) {
        const usedLogger = logger || persistor.logger;
    ...
    // countFromMongoQuery
    export async function countByQuery(persistor, template, query, logger?) {
        const collection = UtilityFunctions.getCollectionByTemplate(persistor, template);
     ...
     * @returns {*}
     */
    export function createSubDocQuery(persistor, targetQuery, targetTemplate, logger) {
     ...
     */
    async function helper(persistor, closureProp, closurePersistorProp, cascadeFetch, schema, defineProperty, obj, promises, query, options, idMap, isTransient, logger) {
     ...
     * getPOJOsFromPaths
     */
    export function getPOJOSFromPaths(persistor, _template, paths, pojo, query) {
        var pathparts;
        var pojos = [];
        for (var ix = 0; ix < paths.length; ++ix) {
    ...
    export async function getFromPersistWithMongoQuery(persistor, template, query, cascade, skip, limit, isTransient, idMap, options?, logger?) {
        idMap = idMap || {};
        options = options || {};
        if (typeof (skip) != 'undefined')
            options.skip = skip * 1;
    ...
    export async function getFromPersistWithMongoId(persistor, template, id, cascade, isTransient, idMap) {
        // old code for reference until unit tests pass
        // return getFromPersistWithMongoQuery(persistor, template, {_id: ObjectID(id.toString())}, cascade, null, null, isTransient, idMap)
        //     .then(function(pojos) { return pojos[0] });

        const objectId = ObjectID(id.toString());
        const results = await getFromPersistWithMongoQuery(persistor, template, { _id: objectId }, cascade, null, null, isTransient, idMap)
