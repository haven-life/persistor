...
function clearCollection(template) {
    ...
    if (collectionName.match(/pg\/(.*)/)) {
        ...
        return Knex.Database.dropKnexTable(PersistObjectTemplate, template)
            .then(function () {
                return Knex.Database.synchronizeKnexTableFromTemplate(PersistObjectTemplate, template).then(function() {return 0});
            });
    } else
...
describe('Banking from pgsql Example', function () {
    ...
    it('countFromKnexQuery using a knex queries in the callback...', function () {
        ...
        return Knex.Database.countFromKnexQuery(PersistObjectTemplate, Customer, func).then(function (count) {
